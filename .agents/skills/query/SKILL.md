...
> **語言政策 (Language Policy):** All agent-generated output — synthesized answers, derived wiki files, and log entries — MUST be written in **Traditional Chinese (繁體中文)**.
...
3. **Synthesize & Render (Generator):** Generate the answer relying strictly on the fetched internal wiki files, prioritizing internal citations over generic external knowledge. **All prose output MUST be in Traditional Chinese (繁體中文).**
   - If the user requested a specific artifact, format the output accordingly (e.g., wrap Marp code in standard markdown code blocks, or output the `matplotlib` Python script). Narrative descriptions and annotations within artifacts must also be in Traditional Chinese.
...
6. **Execute Save & Log:** Upon user approval:
   - Create `wiki/derived/<topic>_analysis.md`. **All content in this file MUST be in Traditional Chinese (繁體中文).**
   - Add a reference link to the new file in `index.md`.
   - Append a creation/query event log to `log.md` in Traditional Chinese.
   - *Fallback:* If write permissions fail, output the raw markdown (in Traditional Chinese) so the user can save it manually.
